(function(){
	...
	function _filterNodes(list, options) {
		...
		for(i = 0, l = list.length; i < l; i++) {
			item = list.item(i);
			if(item.nodeType == COMMENT_NODE && !options.compareComments)
				continue;
			ret.push(item);
		}	
		return ret;
	...
	function _compareNode(left, right, options) {
		...
		if(left.nodeName === right.nodeName && left.nodeType == right.nodeType) {
			switch(left.nodeType) {
				...
				case COMMENT_NODE:
					if(left.nodeType == COMMENT_NODE && !options.compareComments)
						return true;
					vLeft = "" + left.nodeValue, vRight = "" + right.nodeValue;
					if(options.stripSpaces && left.nodeType !== CDATA_SECTION_NODE) {
						vLeft = vLeft.trim();
						vRight = vRight.trim();
					}
					return vLeft === vRight;
